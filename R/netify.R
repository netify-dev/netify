...
#' Constructs a netlet object
#'
#' `get_netify` is a helper function for users to create a netify object.
#' Not visible to users.
#'
#' @param data data object
#' @param netify_type type of netlet object
#' @param actor_pds actor year data object
#' @param actor_time_uniform logical for whether time is uniform
#' @param weight weighted or not
#' @param symmetric logical for whether data is symmetric
#' @param isolates logical for what to do with isolates
#' @param diag_to_NA logical for how to treat diagonal values
#' @param missing_to_zero logical for how to treat missing values
#' @param nodal_data nodal data attribute
#' @param dyad_data dyad data attribute
#' @param graph_data graph data attribute
#' @param ... additional parameters
#'
#' @return netlet object

get_netify <- function(data = matrix(data=NA_real_,nrow=1,ncol=1),
    netify_type = NULL,
    actor_pds = list(NULL),
    actor_time_uniform = TRUE,
    weight = list(NULL),
    ...
    diag_to_NA = TRUE,
    missing_to_zero = TRUE,
    nodal_data = list(NULL),
    dyad_data = list(NULL),
    graph_data = list(NULL), 
    ...
    ){

  validate_netify(new_netify(data,
    netify_type = netify_type,
    actor_pds = actor_pds,
    actor_time_uniform = actor_time_uniform,
    weight = weight,
...
#' Validates a new netlet object
#'
#' `validate_netify` is a helper function that validates inputs for a user-created netlet.
#' Not visible to users.
#'
#' @param netlet netlet object
#'
#' @return netlet

validate_netify <- function(netlet) {

    ## symmetric -- cor(c(mat), c(t(mat)), use='pairwise.complete.obs' )
    ## weight -- min & max test vs 0
    ## isolates -- rowSums, colSums, to determine if any node has 0 in both
    ## actor_time_uniform -- take in the list of matrices object; Reduce('nrow', matList); check rownames
    stopifnot( "actor_pds should be a dataframe" =
        is.data.frame( attr(netlet,"actor_pds") ) || is.null( attr(netlet,"actor_pds")) )

    stopifnot( "actor_time_uniform should be a logical" =
        methods::is( attr(netlet, "actor_time_uniform"), "logical") )

    stopifnot( "weight should be a character vector indicating a variable from the input data" =
        methods::is( attr(netlet, "weight"), "character") || is.null( attr(netlet,"weight")) )

    stopifnot( "symmetric should be a logical" =
        methods::is( attr(netlet, "symmetric"), "logical") )

    stopifnot( "isolates should be a logical" =
        methods::is( attr(netlet, "isolates"), "logical") )

    stopifnot( "diag_to_NA should be a logical" =
        methods::is( attr(netlet, "diag_to_NA"), "logical") )

    stopifnot( "missing_to_zero should be a logical" =
        methods::is( attr(netlet, "missing_to_zero"), "logical") )
