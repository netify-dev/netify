	...
	"add_dyad_vars: no time ID, asymmetric dyad vars",
	{
		...
		for (v in paste0("var", 1:4)) {
			for (ii in 1:nrow(fake_dyads)) {
				...
				val = fake_dyads[ii, v]
				manual_list[["1"]][[v]][a1, a2] = val
			}
		}

		# test if identical
		expect_equal(attributes(a_matrix)$dyad_data, manual_list)
	}
)
...
test_that(
	"add_dyad_vars: no time ID, symmetric dyad vars",
	{
		...
		gen_symm_var = function(...) {
			mat = matrix(rnorm(9), nrow = 3, ncol = 3, dimnames = list(letters[1:3], letters[1:3]))
			diag(mat) = NA
			mat[upper.tri(mat)] = mat[lower.tri(mat)]
			v_dat = cbind(expand.grid(actor1 = rownames(mat), actor2 = colnames(mat)), var = c(mat))
			v_dat$actor1 = as.character(v_dat$actor1)
			v_dat$actor2 = as.character(v_dat$actor2)
			return(v_dat$var)
		}
		...
		fake_dyads$actor1 = as.character(fake_dyads$actor1)
		fake_dyads$actor2 = as.character(fake_dyads$actor2)
		fake_dyads$var1 = gen_symm_var()
		fake_dyads$var2 = gen_symm_var()
		fake_dyads$var3 = gen_symm_var()
		fake_dyads$var4 = gen_symm_var()
		fake_dyads = fake_dyads[fake_dyads$actor1 != fake_dyads$actor2, ]
		fake_dyads$year = 2312
...
test_that(
	"add_dyad_vars: supplied time ID, asymmetric dyad vars",
	{
		...
		manual_list = lapply(unique(fake_dyads$time), function(time_pd) {
			...
			slice = fake_dyads[fake_dyads$time == time_pd, ]
		...
		names(manual_list) = as.character(unique(fake_dyads$time))

		# test if identical
		expect_equal(manual_list, attributes(a_matrix)$dyad_data)
	}
)
...
test_that(
	"add_dyad_vars: replace existing variables",
	{
		fake_dyads = expand.grid(actor1 = letters[1:3], actor2 = letters[1:3])
		fake_dyads$actor1 = as.character(fake_dyads$actor1)
		fake_dyads$actor2 = as.character(fake_dyads$actor2)
...
test_that(
	"add_dyad_vars: merge with existing dyad data",
	{
		fake_dyads = expand.grid(actor1 = letters[1:3], actor2 = letters[1:3])
		fake_dyads$actor1 = as.character(fake_dyads$actor1)
		fake_dyads$actor2 = as.character(fake_dyads$actor2)
...
# empty data handling
test_that(
	"add_dyad_vars: empty data slices",
	{
...
# bipartite networks
test_that(
	"add_dyad_vars: bipartite networks",
	{
...
test_that(
	"add_dyad_vars: sparse network efficiency",
	{
...
test_that(
	"add_dyad_vars: automatic variable detection",
	{
		fake_dyads = expand.grid(actor1 = letters[1:3], actor2 = letters[1:3])
		fake_dyads$actor1 = as.character(fake_dyads$actor1)
		fake_dyads$actor2 = as.character(fake_dyads$actor2)
