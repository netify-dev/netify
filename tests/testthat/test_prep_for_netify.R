...
test_that("prep_for_netify handles single igraph objects", {
  # single igraph
...
test_that("prep_for_netify handles single network objects", {
  # single network
  nw <- network::network(
    matrix(
...
test_that("prep_for_netify handles single matrices", {
  # Single matrix
  mat <- matrix(runif(25), 5, 5)
  rownames(mat) <- colnames(mat) <- paste0("actor", 1:5)
  net_mat <- prep_for_netify(mat)
...
test_that("prep_for_netify handles 3D arrays (longitudinal)", {
...
test_that("prep_for_netify handles arrays comprehensively", {
  ...
  net_3d_no_names <- prep_for_netify(arr_3d_no_names)
  expect_s3_class(net_3d_no_names, "netify")
  expect_equal(attr(net_3d_no_names, "netify_type"), "longit_array")
})

test_that("prep_for_netify handles list of igraph objects", {
  g <- igraph::make_ring(5)
  igraph::E(g)$myweight <- 1:5
...
test_that("prep_for_netify handles list of network objects", {
  nw <- network::network(
    matrix(
        rbinom(25, 1, 0.3), 5, 5), directed = FALSE)
  network::set.edge.attribute(
...
test_that("prep_for_netify handles list of matrices", {
  mat_list <- list(
...
test_that("prep_for_netify handles lists without names", {
...
test_that("prep_for_netify handles edge cases for lists", {
  ...
  single_g_list <- prep_for_netify(list(g))
  ...
  single_nw_list <- prep_for_netify(list(nw))
...
test_that("prep_for_netify correctly extracts and converts dyad data", {
  ...
  net_g <- prep_for_netify(g, weight = "weight")
...
test_that("prep_for_netify handles vertex attributes correctly", {
  ...
  net_g <- suppressWarnings(prep_for_netify(g))
...
test_that("prep_for_netify handles user-provided nodal and dyad data", {
  g <- igraph::make_ring(5)
  ...
  net_g <- prep_for_netify(g, nodal_data = nodal_data, dyad_data = dyad_data)
...
test_that("prep_for_netify handles bipartite graphs correctly", {
  ...
  net_bip <- prep_for_netify(g_bip, weight = "weight")
  adj_mat <- net_bip
...
test_that("prep_for_netify handles bipartite networks correctly", {
  ...
  net_bip <- prep_for_netify(nw_bip, weight = "weight")
  adj_mat <- net_bip
...
test_that("prep_for_netify handles directed vs undirected networks", {
  # Directed igraph
  g_dir <- igraph::make_ring(4, directed = TRUE)
  igraph::E(g_dir)$weight <- 1:4
  net_dir <- prep_for_netify(g_dir, weight = "weight")
  ...
  net_undir <- prep_for_netify(g_undir, weight = "weight")
...
test_that("prep_for_netify handles weighted networks correctly", {
  # Weighted matrix
  w_mat <- matrix(runif(16), 4, 4)
  diag(w_mat) <- 0
  rownames(w_mat) <- colnames(w_mat) <- paste0("v", 1:4)
  
  net_wmat <- prep_for_netify(w_mat)
  ...
  net_bmat <- prep_for_netify(b_mat)
  expect_true(attr(net_bmat, "weight_binary"))
})

# data checks

test_that("prep_for_netify validates and aligns data correctly", {
...
test_that("prep_for_netify preserves attributes across different input types", {
  ...
  # Matrix version
  net_mat <- prep_for_netify(adj)
  
  # igraph version
  g <- igraph::graph_from_adjacency_matrix(adj, mode = "undirected")
  net_ig <- prep_for_netify(g)
  
  # network version
  nw <- network::network(adj, directed = FALSE)
  net_nw <- prep_for_netify(nw)
...
test_that("prep_for_netify works with complex real-world data structures", {
  ...
  net <- prep_for_netify(mat, nodal_data = nodal_data, dyad_data = dyad_vars)
...
test_that("prep_for_netify error handling", {
  # Unsupported object type
  expect_error(
    prep_for_netify(data.frame(a = 1:5))
  )
  ...
  mat <- matrix(1:9, 3, 3)
  expect_error(
    prep_for_netify(list(g, mat))
  )
  
  # Invalid nodal data
  g <- igraph::make_ring(3)
  expect_error(
    prep_for_netify(g, nodal_data = "not a dataframe")
  )
})