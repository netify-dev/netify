...
if(democracy_homophily$p_value < 0.05) {
  cat("→ Democracies significantly tend to ally with similarly democratic countries\n")
} else {
  cat("→ No significant democracy-based alliance preferences detected\n")
}
```
...
cat("- Same-regime alliances:", round(regime_homophily$mean_similarity_connected, 3), "\n")
cat("- Cross-regime alliances:", round(regime_homophily$mean_similarity_unconnected, 3), "\n")
cat("- P-value:", round(regime_homophily$p_value, 3), "\n")
if(regime_homophily$p_value < 0.05 && regime_homophily$homophily_correlation > 0.1) {
  cat("→ Countries strongly prefer allies with similar political systems\n")
} else {
  cat("→ Regime type doesn't significantly influence alliance formation\n")
}
```

The regime type analysis also reveals a pattern of political homophily in alliance formation. With a homophily score of 0.133 (p < 0.001), countries demonstrate a clear preference for forming alliances with similar regime types. The similarity scores, 41% for allied pairs versus 26.2% for non-allied pairs, indicate that countries sharing the same regime type (Democracy, Hybrid, or Autocracy) are substantially more likely to form alliances. This 14.8 percentage point difference suggests that political regime compatibility plays at least some role in international cooperation.
...
if(gdp_homophily$p_value < 0.05 && gdp_homophily$homophily_correlation > 0) {
  cat("→ Countries at similar development levels are more likely to form alliances\n")
} else {
  cat("→ Economic development levels don't significantly predict alliance patterns\n")
}
```
...
if(geo_correlation$correlation < -0.1 && geo_correlation$p_value < 0.05) {
  cat("✓ Geography matters: Countries form more alliances with nearby nations.\n")
  ...
  cat("✗ Surprising: Greater distance associated with more alliances.\n")
  cat("  (Positive correlation = greater distance, more alliances)\n")
} else {
  cat("→ Geography shows no clear effect on alliance patterns.\n")
...
The `attribute_report()` tries to combines the previous analyses into a comprehensive report. 
...
print(homophily_display[, c("attribute", "method", "homophily_correlation", "p_value", "significance", "interpretation")])
```
...
print(centrality_display[1:10, c("node_var", "centrality_measure", "correlation", "p_value", "interpretation")])
```
...
And as seen in other vignettes we can we use the `plot.netify()` function to visualize the network with node attributes and edge weights:
