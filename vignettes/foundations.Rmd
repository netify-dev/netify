...
# This vignette requires peacesciencer, dplyr, and countrycode.
# Exit gracefully with an actionable message if any are missing.
pkg_check <- c("peacesciencer", "dplyr", "countrycode")
...
This vignette provides a high-level overview of our package from start to finish. It introduces the core components of the package with data examples and minimal writing.
...
*Note: this chunk runs only when the `amen` package is installed (`install.packages("amen")`). If `amen` is not available, the chunk is skipped but the rest of the vignette is unaffected.*

```{r, eval=requireNamespace("amen", quietly = TRUE)}
library(amen)
...
*Note: this chunk runs only when the `ergm` package is installed (`install.packages("ergm")`). If `ergm` is not available, the chunk is skipped but the rest of the vignette is unaffected.*

```{r, eval=requireNamespace("ergm", quietly = TRUE)}
library(ergm)
