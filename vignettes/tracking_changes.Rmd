...
The updated `compare_networks()` function now supports multiple permutation schemes for more sophisticated hypothesis testing:
...
Note that MRQAP methods (Freedman-Lane and DSP) often yield higher p-values as they account for network dependencies.
...
spectral_comp_fast <- compare_networks(
    ...
    method = "spectral",
    spectral_rank = 50  # Use only top 50 eigenvalues
)
```

The top eigenvalues capture the most significant structural features, so using 50-100 eigenvalues often provides a good balance between accuracy and performance.
...
- `spectral_rank`: Number of eigenvalues for spectral distance (0 = all, default)
