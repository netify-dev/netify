...
# any harder for ergM?
# called prep_for_network because it's a reference
# to the network library, which is what ergm uses
mid_cross_ergm = prep_for_network(mid_cross_network)
...
We show how to use `netify` and UCDP data as just one exaimple of potential applications to intrastate event data. The first step is to go to https://ucdp.uu.se/downloads/ and download the data you want to use. For this tutorial we have downloaded UCDP GED event data version 23.1 and subset the data for the case of Mexico. 
