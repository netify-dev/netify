...
```{r}
peek(mid_long_network, when_to_peek = c('2012'), what_to_peek = c(13, 15:24))
```
...
We might also want to look at a summary of actor statistics overtime. Our built-in function, `summary_actor` will calculate in-degree, out-degree, average degree, and eigenvector centrality for each actor in each time period.

```{r}
# every year & every actor (messy!) but check out help file
summary_actor_mids <- summary_actor(mid_long_network)
head(summary_actor_mids)
```

We can take a single actor and examine their statistic over time:

```{r, fig.width=6, fig.height=6}
actor_2 <- summary_actor_mids[summary_actor_mids$actor == 2, ]

ggplot(actor_2, aes(x = time, y = closeness)) +
  geom_line() +
  labs(x = "Time", y = "Closeness Centrality")
```

Or we can look at the distribution of the statistic for all actors over time:

```{r, fig.width=6, fig.height=6}
# violin plot across all actors
# and for each stat
plot_actor_stats(
  summary_actor_mids 
  )
...
And then select a specific statistic to focus on:
```{r, fig.width=6, fig.height=6}
# focus on closeness
plot_actor_stats(
  summary_actor_mids,
...
Say that we wanted to showcase actor-specific statistics over time. We can use the plot_actor_stats function for this as well, though it's recommended to subset to a few actors for a more legible plot.
...
Once we have created and explored our network object, we might want to continue analyzing the data using different modeling approaches. `netify` makes this simple even though those models aren't! And for the sake of convergence/degeneracy hell lets go with cross-sectional networks. 
...
ergm_model <- ergm(
  formula = mid_cross_ergm ~ 
    edges + edgecov(mid_cross_ergm, 'capdist_e') +
    nodecov("polity21") + 
    nodecov("polity22") +
    nodecov("wbgdp2011est1") + nodecov("wbgdp2011est2") +
    nodematch("wbpopest1") + nodecov("wbpopest2")
  )
```
